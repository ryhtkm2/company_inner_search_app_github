...
def display_initial_ai_message():
    ...
    with st.chat_message("assistant"):
        ...
        #container.
        #st.markdown(''':green-background[こんにちは。私は社内文書の情報をもとに回答する生成AIチャットボットです。上記で利用目的を選択し、画面下部のチャット欄からメッセージを送信してください。]''')
        html_greeting = '''こんにちは。私は社内文書の情報をもとに回答する生成AIチャットボットです。
        上記で利用目的を選択し、画面下部のチャット欄からメッセージを送信してください。'''
        html_warning = ''' 具体的に入力したほうが期待通りの回答を得やすいです。'''


        container.markdown(f'''<div style="background-color: rgb(193, 225, 193,0.4); color:#097969; padding: 20px; border-radius: 5px;">{html_greeting}</div>''',unsafe_allow_html=True)

        container.markdown(f'''<div style="background-color: #FFF8DC; color:#8B8000; padding: 15px; border-radius: 5px; margin: 2% 0;"><span>&#9888;</span>{html_warning}</div>''',unsafe_allow_html=True)
                    ...
                    if not "no_file_path_flg" in message["content"]:
                        ...
                        if "main_page_number" in message["content"]:
                            ...
                            if main_page_number == 0:
                                main_page_number = '0〜1'
                            st.success(f"{message['content']['main_file_path']} (ページNo.{main_page_number})", icon=icon)
                        else:
                            st.success(f"{message['content']['main_file_path']} です。", icon=icon)
                        ...
                        if "sub_message" in message["content"]:
                            ...
                            for sub_choice in message["content"]["sub_choices"]:
                                ...
                                if "page_number" in sub_choice:
                                    ...
                                    st.info(f"{sub_choice['source']}です。", icon=icon)
        ...
        if sub_choices:
            ...
            for sub_choice in sub_choices:
                ...
                if "page_number" in sub_choice:
                    ...
                    #st.info(f"{sub_choice['source']}", icon=icon)
                    #【問題4】参照先ページ数の表示,1ページ目を0と表示することがあるので。
                    page_number = sub_choice["page_number"]
                    if page_number == 0:
                        page_number = '0〜1'
                    st.info(f"{sub_choice['source']} (ページNo.{page_number})", icon=icon)
        ...
        for document in llm_response["context"]:
            ...
            if "page" in document.metadata:
                ...
                if page_number == 0:
                    page_number = '0〜1'
                file_info = f"{file_path} (ページNo.{page_number})"
                print(f"{file_path} (ページNo.{page_number})")
            else:
                # 「ファイルパス」のみ
